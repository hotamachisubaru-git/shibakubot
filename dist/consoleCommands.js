...
const DURATION_ERROR_MESSAGE = "duration は 例: 10s, 5m, 2h, 300 (秒) の形式で指定してください。";
...
function printConsoleCommandHelp() {
...
async function moveUser(client, guildId, userId, channelId) {
...
async function disconnectUser(client, guildId, userId) {
    const guild = await fetchGuild(client, guildId);
    if (!guild) {
        return;
    }
...
async function timeoutUser(client, guildId, userId, durationMs, label) {
    const guild = await fetchGuild(client, guildId);
    if (!guild) {
        return;
    }
    ...
    const humanDuration = label ?? `${durationMs / 1000}秒`;
    console.log(`✅ ${member.user.tag} を ${humanDuration} タイムアウトしました。`);
}
async function serverUserMute(client, guildId, userId, durationMs, label) {
    const guild = await fetchGuild(client, guildId);
    if (!guild) {
        return;
    }
    ...
    try {
        ...
        const humanDuration = label ?? `${durationMs / 1000}秒`;
        console.log(`✅ ${member.user.tag} を ${humanDuration} サーバーミュートしました。`);
        if (durationMs > 0) {
            setTimeout(() => {
...
async function autoUnmuteUser(guild, userId) {
    try {
        const member = await guild.members.fetch(userId).catch(() => null);
...
async function moveAll(client, guildId, targetChannelId) {
    const guild = await fetchGuild(client, guildId);
    if (!guild) {
        return;
    }
...
async function disconnectAll(client, guildId) {
    const guild = await fetchGuild(client, guildId);
    if (!guild) {
        return;
    }
...
async function muteAll(client, guildId, durationMs, label) {
    const guild = await fetchGuild(client, guildId);
    if (!guild) {
        return;
    }
...
async function autoUnmuteAll(guild) {
    try {
        let unmutedCount = 0;
        for (const voiceState of guild.voiceStates.cache.values()) {
            const member = voiceState.member;
...
async function addRoleToUser(client, guildId, userId, roleId) {
    const guild = await fetchGuild(client, guildId);
    if (!guild) {
        return;
    }
...
async function executeConsoleCommand(client, input) {
