      ...
      setTimeout(async () => {
        try {
          ...
          if (refreshed.voice?.channel) {
            ...
            console.log(
              `✅ ${refreshed.user.tag} のサーバーミュートを解除しました。`,
            );
          }
        } catch (err) {
  ...
  try {
    if (command === "move" && args.length === 4) {
      ...
      if (member.roles.cache.has(role.id)) {
        console.log(
          `${member.user.tag} はすでにロール ${role.name} を持っています。`,
        );
        return;
      }
      ...
      console.log(
        `✅ ${member.user.tag} にロール ${role.name} を付与しました。`,
      );
    } else if (command === "help") {
      console.log("利用可能なコマンド:");
      console.log(
        "  move <guildId> <userId> <voiceChannelId>   - ユーザーを指定のVCに移動",
      );
      console.log(
        "  disconnect <guildId> <userId>              - ユーザーをVCから切断",
      );
      console.log(
        "  timeout <guildId> <userId> <second(s)/minute(s)/hour(s)>    - ユーザーをタイムアウト（0以下で解除）",
      );
      console.log(
        "  serverMute <guildId> <userId> <second(s)/minute(s)/hour(s)> - サーバーミュート（0以下なら解除なし）",
      );
      console.log(
        "  moveAll <guildId> <voiceChannelId>         - ギルド内の全VC参加者を指定VCへ移動",
      );
      console.log(
        "  disconnectAll <guildId>                    - ギルド内の全VC参加者を切断",
      );
      console.log(
        "  muteAll <guildId> <second(s)/minute(s)/hour(s)>             - ギルド内の全VC参加者を一括サーバーミュート",
      );
      console.log(
      ...
      console.log(
        "  delmsg <channelId> <messageId>             - メッセージを削除",
      );
    } else if (command) {
