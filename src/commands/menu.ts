...
import {
  ...
  getMedalBalance,addMedals,setMedals
} from '../data';
...
function buildMenu(min: number, max: number) {
  ...
  const row3 = new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    new ButtonBuilder().setCustomId('menu_ranking').setLabel('ランキング').setStyle(ButtonStyle.Secondary)  
  );
      ...
      switch (btn.customId) {
        ...
        case 'menu_sbk': {
          ...
          sub.on('collect', async (i) => {
            if (i.isUserSelectMenu() && i.customId === 'sbk_pick_user') {
              ...
              pickedCount = Math.max(sbkMin, Math.min(sbkMax, Number(i.values[0] ?? sbkMin)));
              await i.deferUpdate();
        ...
        case 'menu_immune': {
          ...
          sub.on('collect', async i => {
            if (i.isStringSelectMenu() && i.customId === 'imm_act') {
              ...
              await i.deferUpdate();
            } else if (i.isUserSelectMenu() && i.customId === 'imm_user') {
              target = i.values[0] ?? null;
              await i.deferUpdate();
            } else if (i.isButton() && i.customId === 'imm_cancel') {
              await i.update({ content: 'キャンセルしました。', components: [] });
              sub.stop('cancel');
              ...
              if (act === 'list') {
                ...
                await i.reply({
                  content: list.length ? list.map((x, n) => `${n + 1}. <@${x}> (\`${x}\`)`).join('\n') : '（なし）',
                  ephemeral: true
                });
              } else if (act === 'add') {
        ...
        case 'menu_control': {
          ...
          sub.on('collect', async i => {
            if (i.isUserSelectMenu() && i.customId === 'ctl_user') {
              target = i.values[0] ?? null;
              await i.deferUpdate();
            } else if (i.isButton() && i.customId === 'ctl_cancel') {
              await i.update({ content: 'キャンセルしました。', components: [] });
              sub.stop('cancel');
        ...
        case 'menu_vcdisconnect': {
          ...
          break;
                ...
                /* --- メダルバンク（自分の残高を見るだけの簡易版） --- */
        case 'menu_bank': {
          await btn.deferUpdate();

          const balance = await getMedalBalance(btn.user.id); // data.ts の関数
...
case 'menu_admin': {
      ...
      modal.addComponents(
        new ActionRowBuilder<TextInputBuilder>().addComponents(
          new TextInputBuilder()
            ...
            .setStyle(TextInputStyle.Short)
            .setRequired(true)
            .setLabel(mode === 'set' ? '新しい残高（0以上の整数）' : '増減する枚数（+/- の整数）')
        )
      );
        ...
        case 'menu_help': {
          ...
          await btn.followUp({
            embeds: [new EmbedBuilder()
              .setTitle('ヘルプ')
              .setDescription([
                'このメニューから、ランキング/メンバー/統計/しばく/ルーム告知/上限設定/免除管理/値の直接設定/VC移動/VC切断 が使えます。',
                '※ 上限設定・免除管理・値の直接設定・VC移動・VC切断は 管理者 or OWNER_IDS で利用可。',
  ...
  collector.on('end', async () => {
    try {
      await (msg as any).edit({ components: disabledCopyOfRows(built.rows) }).catch(() => {});
    } catch {}
  });
}
